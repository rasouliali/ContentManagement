...
using CM.Application.UseCases.UserManager.Queries.SignOut;
using CM.UI.Controllers;
using CM.UI.Helpers;
using CM.UI.Models;
...
namespace CM.UI.Repositories
{
    public class AccountRepository
    {
        ...
        internal async Task<(UserLoginDto, string)> LoginAsync(SignInQuery input)
        {
            try
            {
        ...
        internal async Task<bool> SignUpAsync(UserLoginDto input)
        {
            try
            {
                var json = JsonConvert.SerializeObject(input);
